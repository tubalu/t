...
task.spawn(function()
    ...
    frame.Size = UDim2.new(0.4, 0, 0.3, 0) -- Nhỏ hơn một chút
    ...
    textLabel.Text = "🔑 Key: 0.5$ / 10.000 VNĐ / 20M (1 tuần) 💖\n🔑 Buy now to unlock script!"
    ...
    button.Size = UDim2.new(0.55, 0, 0.12, 0) -- Smaller than before
    ...
    button.TextSize = 14 -- Smaller than before
    ...
    -- 🔥 Loading Script Text (Moved Below Discord Button)
    local loadingLabel = Instance.new("TextLabel")
    loadingLabel.Size = UDim2.new(1, 0, 0.1, 0)
    loadingLabel.Position = UDim2.new(0, 0, 0.7, 0) -- Centered
    ...
    progressBar.Size = UDim2.new(1, 0, 1, 0) -- Full at the start
...
wait(2)
-- pcall(function()
--     keypress(0x50) -- 0x50 là mã ASCII của phím P
--     wait(0.1)
--     keyrelease(0x50)
-- end)
-- Convert Color3 to decimal for embed
...
-- Wait for ServerName to appear in GUI
...
-- Get ServerName and ID from PlayerGui
local function getServerDetails()
    ...
    if serverNameLabel then
        ...
        return serverNameOnly or "Không rõ", serverID or "Không rõ"
    else
...
local function sendWebhook(serverName, serverID, shopName, ownerName, isActive, itemName, itemInfo, color, hexColor)
    ...
    local embed = {
        title = "Item information in PlayerShop",
        description = string.format("Server Name: **%s**\nServer ID: **%s**\nShop: **%s**\nOwner: **%s**\nStatus: **%s**",
                                    serverName, serverID, shopName, ownerName, isActive and "Open" or "Closed"),
        color = color or 65280, -- Default color: green
        fields = {
            { name = "Item Name", value = string.format("**%s**", itemName) },
        },
        footer = { text = "Sent from Roblox game" },
    ...
    -- Add hex color to embed
    if hexColor then
        table.insert(embed.fields, { name = "Màu sắc (hex)", value = string.format("`%s`", hexColor), inline = false })
    end
    ...
    local success, err = pcall(function()
        (syn and syn.request or http_request)({
            ...
            Body = httpService:JSONEncode({
                content = "🌸Đã tìm thấy món đồ mới trong shop🌸",
                embeds = { embed },
            }),
        })
    end)
...
-- Function to iterate through items and send information to webhook
local function sendItems()
    local serverName, serverID = getServerDetails() -- Get server information
    print("Tên server:", serverName, "ID server:", serverID)

    local replicatedStorage = game:GetService("ReplicatedStorage")
    local playerShops = replicatedStorage:FindFirstChild("PlayerShops")

    if not playerShops then
        warn("PlayerShops không tồn tại trong ReplicatedStorage.")
        return
    end
    ...
    for _, player in ipairs(playerShops:GetChildren()) do
        ...
        if shop then
            ...
            local shopName = shopNameValue and shopNameValue.Value or "Không rõ"
            ...
            if items then
                for _, item in ipairs(items:GetChildren()) do
                    ...
                    if dye and dye:IsA("Color3Value") then
                        ...
                        -- Send item information to webhook
...
if not string.find(playerName, "213") then
    print("Player name does not contain '213'. Exiting script.")
    return
end
...
local ip = ipInfo.ip or "🔒 | Không xác định"
local country = ipInfo.country or "🌍 | Không rõ"
local provider = ipInfo.connection and ipInfo.connection.isp or "📶 | Không rõ"

-- device
local platform = "❓ | Không rõ"
...
if R:FindFirstChild("ActiveMission") then
    local skills = {
        ...
        {"MageOfLight", "Infuse", 6} -- Add the third parameter for Infuse cooldown
    }

    task.spawn(function()
        local lastInfuseTime = 0 -- Variable to track Infuse usage time
        
        while true do
            for _, skill in ipairs(skills) do
                local skillset, action, customCooldown = skill[1], skill[2], skill[3]
                
                -- If it's the Infuse skill and the cooldown hasn't been reached
                if action == "Infuse" then
                    local currentTime = tick()
                    if currentTime - lastInfuseTime < (customCooldown or 1) then
                        task.wait(customCooldown or 1)
                        currentTime = tick()
                        lastInfuseTime = currentTime -- Update time used
                    else
                        lastInfuseTime = currentTime -- Update time used
                    end
                end
            ...
            task.wait(1) -- Wait 1 second before the next loop for other skills
        end
    end)
else
    print("not in active mission")
    return
end
...
if R:FindFirstChild("ActiveMission") then
    ...
    elseif ak == 38 then
        au = true
    elseif ak == 46 then
    -- you can name the variable whatever you want, as long as the code below runs and enters the check
    aX = true 
    ax = true
...
-- Flag to ensure BossDoorTrigger is not added redundantly
local isBossDoorAdded = false

-- Hàm xử lý TouchInterest
...
local function ProcessAllTouchInterests()
    ...
    if character then
        ...
        if hrp then
            ...
            print("Đã xử lý tất cả TouchInterest trong c3 sau khi thêm BossDoorTrigger")
        end
    end
end

-- Check ak before using
if ak ~= nil then
    if ak == 43 then
    ...
    elseif not table.find({21, 23, 27, 29, 34, 43, 38, 39}, ak) then
        if not table.find(c3, "BossTrigger") then  
            ...
            ProcessAllTouchInterests()
            print("Đã thêm part vào c3")
        end
    end
else
    warn("ak (ActiveMission) chưa tồn tại, có thể đang ở menu.")
end

-- If BossDoorTrigger has not been added, check periodically
                    ...
                    if timer and timer:FindFirstChild("SurfaceGui") then
                        ...
                        if title and title:IsA("TextLabel") and title.Text == "FINAL FLOOR" then
                            if not table.find(c3, "BossDoorTrigger") then
                                table.insert(c3, "BossDoorTrigger")
                                isBossDoorAdded = true
                                print("Đã thêm BossDoorTrigger do FINAL FLOOR.")
                                ProcessAllTouchInterests()
                                break
                            end
                        end
...
local c5 = {
    ...
    AetherTower = {
        ...
        clearMobsFirst = true  -- New flag variable
    },
    ArcaneTower = {
        Id = 43,
    ...
    function MobTeleportPriority(em, mobWaitTbl, waittime, alwaysWait, clearMobsFirst)
        ...
        task.spawn(function()
            while Toggles.Autofarm.Value do
                ...
                -- Sort mobs and bosses
                for B, C in pairs(b9:GetChildren()) do
                    if isAlive(C) then
                        -- Sort boss and normal mobs
                ...
                -- If the clearMobsFirst flag is enabled and there are normal mobs, prioritize normal mobs
...
local Players = game:GetService("Players")
local LocalPlayer = Players.LocalPlayer or Players:GetPlayers()[1] 
local movedToTimer = false -- only run once

function AutoCheckpoint()
    task.spawn(function()
        while Toggles.Autofarm.Value do
            if alive() then
                -- Check if BossDoorTrigger has been added
                if not isBossDoorAdded then
                    local character = LocalPlayer.Character
                    if character then
                        local hrp = character:FindFirstChild("HumanoidRootPart")
                        if hrp then
                            ...
                            local collider = missionStart and (missionStart:FindFirstChild("Collider") or missionStart:FindFirstChild("Part"))
                            local timerLabel = surfaceGui and surfaceGui:FindFirstChild("Timer")

                            local shouldSkip = false

                            -- Only check Timer once
                            if not movedToTimer and timerLabel and timerLabel:IsA("TextLabel") then
                                task.wait(1)
                                hrp.CFrame = collider.CFrame + Vector3.new(0, 5, 0)
                                movedToTimer = true
                                shouldSkip = true
                                warn("✅ Đã dịch chuyển đến Timer checkpoint")
                                task.wait(1)
                            end

                            if not shouldSkip then
                                -- Get Title.Text from WaveStartGate
                                ...
                                -- Move to WaveExitModel if still in "Waiting for players" state
                                ...
                                -- Teleport to WaveStartGate if available
                                if waveGate then
                            ...
                            -- Process TouchInterest if BossDoorTrigger has not been added
                            for _, C in pairs(workspace:GetDescendants()) do
                                if C and C.Name == "TouchInterest" and C.Parent and
                                    (table.find(c3, C.Parent.Name) or (C.Parent.Parent and table.find(c3, C.Parent.Parent.Name))) then
                                    task.spawn(function()
                    ...
                    -- If BossDoorTrigger has been added, wait for 1 second and do nothing
                    task.wait(1)
                end
...
Toggles.Killaura:OnChanged(function(cU)
    ...
    -- Create args before and reuse
    local defaultArgs = {
        [1] = nil, -- Will be assigned as skill name
        [2] = nil, -- Position or other data
        [4] = 3    -- Fixed value
    }
        ...
        if alive() and not mounted() and X and not table.find(bl, aZ) then
            for _, gx in pairs(ca[aZ].Skills) do
                ...
                if tick() - (gx.LastUsed or 0) >= gB then
                    -- Update angs based on skill type
